...
class VHost implements ts.CompilerHost {
    constructor(public data: Record<string, string | null> = {}, public cwd: string = '/') {
    ...
    getDirectories(path: string): string[] {
        const dirents = vol.readdirSync(path, { withFileTypes: true }) as Dirent[]
    ...
    private resolveFilePath(fileName: string, cwd: string = this.getCurrentDirectory()): string {
        return path.posix.resolve(cwd, fileName)
    }

    writeFile(fileName: string, data: string, _writeByteOrderMark: boolean, onError?: (message: string) => void, _sourceFiles?: ReadonlyArray<ts.SourceFile>): void {
    ...
    getSourceFile(fileName: string, languageVersion: ts.ScriptTarget, onError?: (message: string) => void, shouldCreateNewSourceFile?: boolean): ts.SourceFile | undefined {
        return this.getSourceFileByPath(fileName, this.cwd as ts.Path, languageVersion, onError, shouldCreateNewSourceFile)
    }
    ...
    getSourceFileByPath(fileName: string, path: ts.Path, languageVersion: ts.ScriptTarget, onError?: (message: string) => void, shouldCreateNewSourceFile?: boolean): ts.SourceFile | undefined {
        ...
        if(undefined !== fileContent) {
            ...
            if(`function` === typeof onError) onError(`ENOENT: no such file or directory, stat '${fileName}'`)
    ...
    getDefaultLibFileName(options: ts.CompilerOptions): string {
        return path.resolve(this.getCurrentDirectory(), ts.getDefaultLibFileName(options))
    }
...
export default function createVHost(data: Record<string, string | null> = {}, cwd: string = '/'): VHost {
